...
def func_confusion_matrix(y_test, y_pred):
    ...
    sorted(unique_values)  # updates unique values to be in ascending order
    ...
    if(issubclass(type(y_test[0]), np.integer)): # if values are integers
        ...
        for index in range(0, num_classes):  # for selecting a class to work with
            current_value = unique_values[index]
            possible_string_dict[index] = current_value
            y_test_int[y_test == current_value] = index  # like for loop
