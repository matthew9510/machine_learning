...
def c_selection(x_train, y_train, x_validation, y_validation, kernel_types , debugging=False):
    ...
    max_degree = 10
    max_iter = 10
    #poly_degrees = np.arange(1, max_degree)
    kernel_c_dict = {}
    best_c_dict = {}
    #poly_dict = {}
    ...
    for kernel_type in kernel_types:
        ...
        for iteration in range(max_iter):
            ...
            '''
            if kernel_type == 'poly':
                for curr_degree in poly_degrees:
                    # fit models and append the errors to svm_c_error
                    for c_value in c_range:
                        model = svm.SVC(kernel=kernel_type, C=c_value, degree=curr_degree)
                        ...
                        poly_dict[error] = (c_value, curr_degree)
            ...
            for c_value in c_range:
                model = svm.SVC(kernel=kernel_type, C=c_value)
                model.fit(X=x_train,
                          y=y_train.ravel())  # DataConversionWarning: A column-vector y was passed when a 1d array was expected. Please change the shape of y to (n_samples, ), for example using ravel().
                error = 1. - model.score(x_validation, y_validation)
                svm_c_error.append(error)
            if debugging:
                # Visualize
            ...
            '''
            if kernel_type == 'poly':
                best_degree = poly_dict[min(svm_c_error)][1]
            '''
    ...
    #return kernel_c_dict, best_degree
