...
def relu_two_layer_inference(images, image_pixels, hidden_units, classes, reg_constant=0):
  ...
  # Layer 1
  with tf.variable_scope('Layer1'):  # note that this is a wrapper to distinguish not to share variables
...
def sigmoid_two_layer_inference(images, image_pixels, hidden_units, classes, reg_constant=0):
  '''Build the model up to where it may be used for inference.
  ...
  with tf.variable_scope('Layer1'):  # note that this is a wrapper to distinguish not to share variables
    ...
    # Define the layer's output
    hidden = tf.nn.sigmoid(tf.matmul(images, weights) + biases)
...
def leaky_relu_two_layer_inference(images, image_pixels, hidden_units, classes, reg_constant=0):
  '''Build the model up to where it may be used for inference.
  ...
  with tf.variable_scope('Layer1'):  # note that this is a wrapper to distinguish not to share variables
    ...
    # Define the layer's output
    hidden = tf.nn.leaky_relu(tf.matmul(images, weights) + biases)
...
def relu_three_layer_inference(images, image_pixels, hidden1_units, hidden2_units, classes, reg_constant=0):
  '''Build the model up to where it may be used for inference.
  ...
  with tf.variable_scope('Layer2'):  # note that this is a wrapper to distinguish not to share variables
    # Define the variables
    weights = tf.get_variable(
      name='weights',
  ...
  with tf.variable_scope('output_Layer'):
    # Define variables
    weights = tf.get_variable('weights', [hidden2_units, classes],
                              initializer=tf.truncated_normal_initializer(
...
def report(logits, labels):
  """
  Return Prediction tensor along with correctly_predicted tensor
  """
  with tf.name_scope('report'):
    true_false_prediction = tf.equal(tf.arg_max(logits, 1), labels)
    prediction_matrix = tf.arg_max(logits, 1)

  return true_false_prediction, prediction_matrix