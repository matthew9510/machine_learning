...
def c_selection(x_train, y_train, x_validation, y_validation, kernel_types , debugging=False):
    ...
    :param kernal_c_dict - Store best 'C' value with the least error along side the kernel type in kernel_c_dict, dictionary key is the kernel type
    ...
    :return kernel_c_dict[kernel_type] = (best_c, best_c_error_value)
    """
    ...
    for kernel_type in kernel_types:
        ...
        # creating an array with max_iter random 'c' values, Note: for each iteration we add a random decimal in range [0,1)
        for iteration in range(max_iter):
            svm_c_error = []
            c_range = np.array((np.random.rand(20, )))
            if iteration == 0:
                c_range += iteration + 0.000001  # buffer for a paricular kernal; i think linear
            else:
                c_range += iteration
            ...
            '''
            single_c_index_with_least_error = svm_c_error.index(min(svm_c_error))  # get key
            best_c_dict[c_range[single_c_index_with_least_error]] = svm_c_error[single_c_index_with_least_error] # in n out crange = key

        # Store best 'C' value with the least error along side the kernel type in kernel_c_dict
        best_c_error_value = min(best_c_dict.values())
        best_c = list(best_c_dict.keys())[list(best_c_dict.values()).index(best_c_error_value)]  # THIS IS A POWERFUL LINE OF CODE, casting and listcomprehensions
        kernel_c_dict[kernel_type] = (best_c, best_c_error_value)
